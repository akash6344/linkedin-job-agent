...
from linkedin_agent.llm.role_filter import meets_role_requirement
...
from linkedin_agent.storage.db import already_applied_email, is_seen, save_post
...
def _enrich_post_from_images(post: dict[str, Any], analysis: dict[str, Any]) -> dict[str, Any]:
    """OCR post images when text lacks experience or apply contact; re-analyze."""
    image_urls = list(post.get("image_urls") or [])
    if not image_urls or not needs_image_enrichment(post.get("post_text") or "", analysis):
        return analysis

    print(f"  · Reading {min(len(image_urls), 3)} image(s) for missing details...")
    ...
    print(f"  · Image text extracted ({len(image_text)} chars) — re-analyzing")
...
async def run_pipeline() -> RunSummary:
    ...
    print("  NOTIFY=forms + pending emailed separately")
    print("=" * 60)
    ...
    for post in posts:
        ...
        analysis = _enrich_post_from_images(post, analysis)
        ...
        role_ok, role_reason = meets_role_requirement(post["post_text"], analysis, role_tag)
        if not role_ok:
        ...
        # Defense in depth: never SMTP to prose / multi-address / URL junk.
        if apply_email and not is_valid_apply_email(apply_email):
            print(f"  Skip invalid apply_email: {apply_email!r}")
            apply_email = ""
        ...
        if apply_email:
            ...
            if prior:
                print(
                    f"  Skip (already emailed): {apply_email} "
                ...
                save_post(
                    {
                        **base_record,
                        "status": "skipped",
                        "apply_method": None,
                        "notes": f"Already applied to {apply_email}",
                    }
                )
                summary.skipped += 1
                continue
